...
def ppo(env_name, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
...
def tune_Reacher():
    ...
    logger.configure(args.log_dir, ['csv'], log_suffix='biased-ppo-tune-' + str(args.seed)+ str(args.type))

    returns = []
    for seed in seeds:
        hyperparam = random_search(args.seed)
        checkpoint = 4000
        result = ppo([args.env,args.type], actor_critic=core.MLPActorCritic,
